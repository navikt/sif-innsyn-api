...
class OmsorgspengerutbetalingArbeidstakerHendelseKonsumentIntegrasjonsTest {
    ...
    lateinit var omsorgspengerutbetalingArbeidstakerProducer: Producer<String, Any> // Kafka producer som brukes til å legge på kafka meldinger. Mer spesifikk, Hendelser om mottatte oms-utbetaling-snf søknader.

    companion object {
        private val logger: Logger = LoggerFactory.getLogger(OmsorgspengerutbetalingArbeidstakerHendelseKonsumentIntegrasjonsTest::class.java)
    ...
    @BeforeAll
    fun setUp() {
        omsorgspengerutbetalingArbeidstakerProducer = embeddedKafkaBroker.opprettKafkaProducer()
    }
    ...
    @Test
    fun `Konsumere hendelse om Omsorgspengerutbetaling - Arbeidstaker, persister og tilgjengligjør gjennom API`() {

        // Gitt at ingen hendelser med samme aktørId eksisterer...
        repository.findAllByAktørId(aktørId).ikkeEksisterer()

        // legg på 1 hendelse om mottatt søknad om omsorgspengerutbetaling for arbeidstaker...
        omsorgspengerutbetalingArbeidstakerProducer.leggPåTopic(defaultHendelse(søknadIdKey = "soknadId"), OMP_UTBETALING_ARBEIDSTAKER, mapper)
    ...
    fun `Dersom hendelse med journalpostId og aktørId eksisterer, skip deserialisering av duplikat`() {
        ...
        omsorgspengerutbetalingArbeidstakerProducer.leggPåTopic(hendelse, OMP_UTBETALING_ARBEIDSTAKER, mapper)
        omsorgspengerutbetalingArbeidstakerProducer.leggPåTopic(hendelse, OMP_UTBETALING_ARBEIDSTAKER, mapper)
